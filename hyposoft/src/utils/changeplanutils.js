...
function generateWorkOrder(changePlanID, callback){
    changeplansRef.doc(changePlanID).collection("changes").orderBy("step").get().then(function (querySnapshot) {
        if(querySnapshot.empty){
            callback("There are no changes in the change plan.")
        } else {
            let steps = [];
            querySnapshot.docs.forEach(doc => {
                ...
                if(change === "decommission"){
                    ...
                    if(doc.data().assetID){
                        //override asset id
                        steps.push("Add asset #" + doc.data().assetID + " to datacenter " + doc.data().changes.datacenter.new + " on rack " + doc.data().changes.rack.new + " at height " + doc.data().changes.height.new + " U.");
                    } else {
                        //autogenerated
                    }
                } else if(change === "edit"){
                    if(doc.data().changes.datacenter){
                        //datacenter changed
                        steps.push("Move asset #" + doc.data().assetID + " from datacenter " + doc.data().changes.datacenter.old + " on rack " + doc.data())
                    }
...
function generateEditWorkOrderMessage(doc, callback){
    assetRef.doc(doc.data().assetID).get().then(function (documentSnapshot) {
        ...
        if(documentSnapshot.exists){
            if(doc.data().changes.datacenter){
                //datacenter changed
                changes.push("Move asset #" + doc.data().assetID + " from datacenter " + doc.data().changes.datacenter.old + " on rack " + documentSnapshot.data().rack + " at height " + documentSnapshot.data().rackU +
                " U to datacenter " + doc.data().changes.datacenter.new + " on rack " + (doc.data().changes.rack ? doc.data().changes.rack.new : documentSnapshot.data().rack) + " at height " +
                    (doc.data().changes.rackU ? doc.data().changes.rackU.new : documentSnapshot.data().rackU));
            }
            else if(doc.data().changes.rack){
                changes.push("Move asset #" + doc.data().assetID + " in datacenter " + documentSnapshot.data().datacenter + " from rack " + doc.data().changes.rack.old + " at height " + documentSnapshot.data().rackU + " to rack " + doc.data().changes.rack.new +
                " at height " + (doc.data().changes.rackU));
            }
            else if(doc.data().changes.rackU){
                changes.push("Move asset #" + doc.data().assetID + " in datacenter " + documentSnapshot.data().datacenter + " on rack " + documentSnapshot.data().rack + " from height " + doc.data().changes.rackU.old + " U to height " +
                doc.data().changes.rackU.new + " U")
            }
            if(doc.data().changes.networkConnections){
