...
class EditOfflineStorageForm extends React.Component {
    ...
    handleSubmit(event){
        if(!this.state.name || !this.state.abbreviation){
            ...
            offlinestorageutils.updateStorageSite(this.state.oldName, this.state.oldAbbreviation, this.state.name.trim(), this.state.abbreviation.trim(), result => {
                if(result) {
                    this.props.parentCallback("Successfully edited the offline storage site!");
                } else {
                    ToastsStore.error("Error updating the offline storage site. Please ensure that the name and abbreviation are unique.");
                }
            })
        }
    }
...
export default EditOfflineStorageForm